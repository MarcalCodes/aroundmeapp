...
-- Re-doing the `20250427072551_seed_areas_aus.sql` migration because the previous one was containing duplicates

-- Empty the table
-- comes from https://stackoverflow.com/a/71918057/29476271
TRUNCATE TABLE area RESTART IDENTITY CASCADE;

-- Add a unique constraint so that this issue will not happen again
ALTER TABLE area
ADD UNIQUE (suburb, state, postcode);
